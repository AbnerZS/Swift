...
NSString *const ZSCollectionElementKindSectionHeader = @"ZSCollectionElementKindSectionHeader";
NSString *const ZSCollectionElementKindSectionFooter = @"ZSCollectionElementKindSectionFooter";

@interface ZSWaterfullLayout ()
/// The delegate will point to collection view's delegate automatically.
@property (nonatomic, weak) id <ZSCollectionViewLayoutDelegte> delegate;
...
@implementation ZSWaterfullLayout
...
- (void)prepareLayout {
    ...
    self.delegate = (id <ZSCollectionViewLayoutDelegte> )self.collectionView.delegate;
    NSAssert([self.delegate conformsToProtocol:@protocol(ZSCollectionViewLayoutDelegte)], @"UICollectionView's delegate should conform to CHTCollectionViewDelegateWaterfallLayout protocol");
    ...
    for (NSInteger section = 0; section < numberOfSections; ++section) {
        ...
        if (headerHeight > 0) {
            attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:ZSCollectionElementKindSectionHeader withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
        ...
        if (footerHeight > 0) {
            attributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:ZSCollectionElementKindSectionFooter withIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForSupplementaryViewOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([kind isEqualToString:ZSCollectionElementKindSectionHeader]) {
        attribute = self.headersAttribute[@(indexPath.section)];
    } else if ([kind isEqualToString:ZSCollectionElementKindSectionFooter]) {
